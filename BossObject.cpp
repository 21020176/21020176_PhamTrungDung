...
void BossObject::CenterEntityOnMap(Map& g_map)
{
    ...
    else if (g_map.start_x_ + SCREEN_WIDTH >= g_map.map_x_)
    {
        g_map.start_x_ = g_map.map_x_ -SCREEN_WIDTH;
    }
    ...
    else if (g_map.start_y_ + SCREEN_HEIGHT >= g_map.map_y_)
    {
        g_map.start_y_ = g_map.map_y_ - SCREEN_HEIGHT;
    }
}
...
void BossObject::CheckToMap(Map& g_map)
{
    ...
    else if (x_pos_ + width_frame_ >= g_map.map_x_)
    {
        x_pos_ = g_map.map_x_ - width_frame_ -1 ;
    }

    if(y_pos_ > g_map.map_y_)
    {
        think_time_ = 60 ;
    }
}
...
void BossObject::InitBullet(SDL_Renderer* screen)
{
    ...
    bool ret = p_bullet->LoadImg("boss_bullet.png", screen);
    if(ret)
    {
        p_bullet->set_bullet_dir(BulletObject::DIR_LEFT);
        p_bullet->set_is_move(true);
        p_bullet->SetRect(rect_.x - 50, rect_.y + height_frame_/2 + height_frame_/4);
